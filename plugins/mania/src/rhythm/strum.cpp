...
namespace mania {
    Strum::Strum(vsrg::EngineContext* ctx, const std::string& spritePath, int column)
        ...
        ctx->get_debugger()->log(vsrg::DebugLevel::INFO, 
            "strum constructor body started!!", 
            __FILE__, __LINE__);
        ...
        ctx->get_debugger()->log(vsrg::DebugLevel::INFO, 
            "strum constructor finished, dims: " + 
            std::to_string(getDimensions().x) + "x" + std::to_string(getDimensions().y),
            __FILE__, __LINE__);
    }
