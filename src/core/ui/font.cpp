...
namespace vsrg {
	...
	Font::Font(EngineContext* engine_context, FT_Library ft, const std::string& path, int size_pt) : engine_context(engine_context), size_pt(size_pt) {
		...
		// Set baseline height based on 'H' (tall character)
	...
	bool Font::createNewAtlas() {
		...
		engine_context->get_debugger()->log(DebugLevel::INFO,
			std::string("Creating new font atlas of size ") + std::to_string(new_atlas.width) + "x" + std::to_string(new_atlas.height),
			__FILE__, __LINE__);
		...
		if (glGetError() != GL_NO_ERROR) {
			engine_context->get_debugger()->log(DebugLevel::ERROR,
				std::string("Failed to create a new font atlas texture."),
				__FILE__, __LINE__);
	...
	bool Font::loadGlyph(Charcode charcode) {
		...
		while (true) {
			...
			// Check if glyph fits in the current row
			...
			// Check if glyph fits in the atlas height
		...
		// Upload glyph to the atlas texture
		if (glyph->bitmap.width > 0 && glyph->bitmap.rows > 0) {
			...
			glTexSubImage2D(
				GL_TEXTURE_2D,
				0,
				current_atlas->current_x,
				current_atlas->current_y,
				glyph->bitmap.width,
				glyph->bitmap.rows,
				GL_RED,
				GL_UNSIGNED_BYTE,
		...
		Character character = {
			current_atlas->texture_id,
			glm::ivec2(glyph->bitmap.width, glyph->bitmap.rows),
			glm::ivec2(glyph->bitmap_left, glyph->bitmap_top),
			static_cast<GLuint>(glyph->advance.x >> 6),
			glm::vec2((float)current_atlas->current_x / current_atlas->width, (float)current_atlas->current_y / current_atlas->height),
			glm::vec2((float)glyph->bitmap.width / current_atlas->width, (float)glyph->bitmap.rows / current_atlas->height)
		};
		characters.insert({ charcode, character });

		// Update current atlas packing position
