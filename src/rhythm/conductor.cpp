...
namespace vsrg
{
    Conductor::Conductor(AudioManager* audio_manager, Audio *audio, std::vector<TimingPoint> timing_points) : 
    ...
    void Conductor::update(float delta_time)
    {
        ...
        if (current_point)
        {
            ...
            double seconds_per_subdivision = (240.0 / current_point->bpm) / (double)current_point->denominator;
